<template>
  <div class="app-container">
    ...
    <el-row :gutter="20" class="tools-wrapper">
      ...
      <el-col :lg="10" :xs="24">
        <el-radio-group v-model="query.status" class="tools-item" @change="search">
          <el-radio-button v-for="(state, index) in depositStatusTypes.TypesArray" :key="index" :label="state.key">{{ state.value }}</el-radio-button>
        </el-radio-group>
      </el-col>
      <el-col :lg="3" :xs="8">
    ...
    <!-- Table -->
    <el-tabs v-model="coinType" type="border-card" @tab-click="getDepositList">
      <el-tab-pane v-for="(type, index) in coinTypes.TypesArray" :key="index" :name="type.key" :label="type.value">
        ...
        <el-table v-loading="isloading" :data="dataList" stripe border>
          ...
          <el-table-column prop="existentialCoinAddress" label="存币地址" align="center" />
          <el-table-column prop="existentialCoinNum" label="数量" align="center" />
          <el-table-column prop="existentialCoinType" label="币种" align="center" />
          <el-table-column label="状态" align="center">
            <template slot-scope="scope">
              {{ depositStatusTypes[scope.row.existentialCoinState] }}
            </template>
          </el-table-column>
...
<script>
import { getDepositList, remark } from '@/api/wallet'
import { CoinTypes, DepositStatusTypes } from '@/utils/enums'

const modelName = 'DepositList'
...
export default {
  ...
  data() {
    return {
      ...
      coinType: '-1',
      coinTypes: CoinTypes,
      depositStatusTypes: DepositStatusTypes,
      dataList: [],
      remarkDialog: false,
      remarkForm: {
        id: '',
